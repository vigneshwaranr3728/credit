...
import {
  BorrowerProfile, RiskMetrics, RiskLevel,
  RiskFactor, PositiveFactor
} from '../types/index.js';

export function calculateMonthlyEMI(principal: number, annualRate: number, months: number): number {
  const r = annualRate / 12 / 100;
  if (r === 0) return principal / months;
  return (principal * r * Math.pow(1 + r, months)) / (Math.pow(1 + r, months) - 1);
}

export function getInterestRate(profile: BorrowerProfile): number {
  let baseRate = 10;
  if (profile.creditScore >= 750) baseRate = 8;
  else if (profile.creditScore >= 700) baseRate = 10;
  else if (profile.creditScore >= 650) baseRate = 13;
  else if (profile.creditScore >= 600) baseRate = 16;
  else baseRate = 20;

  if (profile.employmentType === 'salaried') baseRate -= 0.5;
  if (profile.collateral) baseRate -= 1;
  if (profile.repaymentHistory === 'excellent') baseRate -= 0.5;
  if (profile.repaymentHistory === 'poor') baseRate += 2;

  return Math.max(7, Math.min(24, baseRate));
}

export function analyzeRisk(profile: BorrowerProfile): RiskMetrics {
...
export function generateSampleProfiles(): BorrowerProfile[] {
  return [
    {
